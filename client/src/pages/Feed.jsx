...
const Feed = () => {
  ...
  // Handle URL parameters for specific posts (coming from notifications)
  useEffect(() => {
    if (feeds.length > 0) {
      ...
      if (postId) {
        ...
        const targetPost = feeds.find(post => post._id === postId)
        ...
        if (targetPost) {
          ...
          setTimeout(() => {
            ...
            if (postElement) {
              ...
              // If there's a specific comment, try to highlight it too
              if (commentId) {
                setTimeout(() => {
          ...
          // Post not found in current feed, could be from a different user
          toast.info('Post not found in your current feed')
          navigate('/feed', { replace: true })
        }
      }
    }
  }, [feeds, location.search, navigate])
                    ...
                    feeds.map((post, index) => (
                      <div
                        ...
                        className={`bg-white lg:rounded-xl lg:shadow-sm border-b border-gray-200 lg:border-none transition-all duration-300 hover:lg:shadow-md ${
                          ...
                          highlightedPost === post._id ? 'ring-2 ring-blue-500 lg:ring-4 lg:ring-blue-400 lg:ring-opacity-50' : ''
                        }`}
                      >
                        <PostCard post={post} />
                      </div>
